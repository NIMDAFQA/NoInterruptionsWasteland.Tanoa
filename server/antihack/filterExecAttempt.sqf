...
if (_packetName == "BIS_fnc_MP_packet") then
{
	...
	if (_function == "BIS_fnc_execVM") then
	{
		...
		[
			...
			"addons\airstrikes\airstrikes_cli_menu.sqf",
			"addons\airstrikes\airstrikes_cli_start.sqf",
			"addons\airstrikes\airstrikes_srv_start.sqf",
			"addons\airstrikes\init.sqf",
			"addons\airstrikes\config.sqf"
		];
	}
	else
	{
